...
SHELL-INTEGRATION GLUE (PATH/aliases/env + the conf.d loaders) is a SEPARATE concern —
the `via: glue` driver (drivers/glue.py). This driver manages a component's own reposited
config only.
...
class DotFiles(Driver):
    ...
    def _specs(self, rc):
        '''[(name, src, dst, absorb)] link specs. A component may mix a single inline spec (top-level
        src/dst) with named specs (`config: {src,dst}`, `aliases: {src,dst}`, ...). `absorb-into`
        relocates a pre-existing real dst instead of a plain backup.'''
        f = rc.fields
        out = []
    ...
    def _resolve(self, src, rc):
        ...
        with tier 'user' or 'template'. The content sits under that root's `<component>.cfs/<src>`
        marker dir (with a legacy bare `<root>/<src>` fallback so content captured before the .cfs
        layout still links). If none has it, the defining-layer path with tier None: the component
        is UNPOPULATED — declared but no content anywhere (a personal dotfile you haven't captured).
        That is an expected state, not an error. `root` is always the BASE content root (for status
        labeling); the returned path points inside `.cfs`.'''
    ...
    def _config_specs(self, rc):
        '''Config specs, EXCLUDING legacy shell-glue snippets declared the old way with an inline
        src/dst under bash.d/ or shell/ — those belong to the shell layout (the glue driver), so
        they never get a .cfs marker/manifest entry or a capture.'''
        return [(n, src, dst, ab) for n, src, dst, ab in self._specs(rc)
                if not (src.startswith('bash.d/') or src.startswith('shell/'))]
    ...
    def _ensure_marker(self, rc):
        ...
        edited; never overwrites content. No-op for a component with no config specs.'''
    ...
    def _store_path(self, src):
        ...
        `src` (nvim/init.lua -> ~/.config/configsys/dotfiles/nvim/init.lua). None if no store.
    ...
    def _materialize_to(self, srcpath, dest):
        '''Copy `srcpath` to `dest` (idempotent — only when `dest` is absent). Returns dest (or
        srcpath if there's nothing to copy / no dest).'''
    ...
    def spec_states(self, rc):
        ...
        `kind` is always 'config' (content you own). state is one of:
          linked    — our symlink is in place (managed & active)
          adopted   — your content exists in a user root; not linked yet (capture done)
          managed   — `.cfs` marker exists but no content captured yet (managed-when-empty, #5 — the
                      component is installed; if a real on-system file sits at dst it's kept and
                      startup warns you to capture it)
        ...
        for name, src, dst, _absorb in self._specs(rc):
            ...
            if srcpath.exists() and os.path.realpath(str(tgt)) == os.path.realpath(str(srcpath)):
                ...
                state = ('adopted' if tier == 'user'
                         else 'managed' if marker else 'unmanaged')
            elif tier == 'user':
            ...
            if state in ('linked', 'adopted', 'template'):
                ...
                rel = f'{rc.comp}{CFS_SUFFIX}/{src}'
            out.append((name, self.display_path(tgt), state, src_root, rel, here, 'config'))
        return out
    ...
    def get_version(self, rc):
        ...
        `.cfs` marker exists (managed-when-empty, #5 — the component IS installed, content just isn't
        captured/linked yet); else None (not installed).'''
        specs = self._specs(rc)
        if not specs:
            return None
        all_linked = True
        for _name, src, _dst, _absorb in specs:
            srcpath, _tier, _root = self._resolve(src, rc)
    ...
    def warnings(self, rc):
        '''[(tag, text)] warn-only startup checks (design #2/#5/#4), never mutate:
          * capture   — a `.cfs` marked-managed whose dst holds a real, un-captured file
        ...
        for _name, src, dst, _absorb in self._specs(rc):
            srcpath, _tier, _root = self._resolve(src, rc)
            tgt = self._expand(dst)
            ...
            elif not srcpath.exists() and tgt.exists():
                if managed is None:
    ...
    def install(self, rc):
        ...
        # content is captured/linked. Runs even if we refuse below — refusing is itself an act of
        # management (we won't clobber what we now track).
        self._ensure_marker(rc)
        pairs, blocked = [], []
        for _name, src, dst, absorb in specs:
            srcpath, tier, _root = self._resolve(src, rc)
            # A config TEMPLATE links to its store copy, so a link never references the repo (#4).
        ...
        # below points at a user-owned copy and never into the repo. A config template lands at its
        # store path.
        ...
        for _srcpath, _tier, src, link_src, tgt, absorb in pairs:
            # If the deploy target already RESOLVES to the store copy, the store file IS the deployed
            # file, and `ln -sfn store/x x` would resolve to a self-link (ELOOP). Skip the link/backup.
            try:
            ...
            if absorb is not None:
                # a pre-existing real dst is RELOCATED (made executable) — the user's own file isn't
                # zapped. If the absorb target is already taken, fall back to a plain backup.
