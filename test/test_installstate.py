...
def flatpak_unit():
    return ResolvedComponent(key='flatpak\\firefox', family='flatpak', comp='firefox',
                             fields={'name': 'org.mozilla.firefox'})
...
def test_unsupported_family_degrades():
    fr = FakeRunner()
    led = Ledger()
    led.set_managed('flatpak\\firefox', True)
    st = InstallState(fr, led).inspect_one(flatpak_unit())
    assert not st.supported
    assert st.status == 'unsupported'
    assert st.managed is True
    assert 'flatpak' in st.error
...
def test_inspect_many():
    ...
    units = {'apt\\btop': apt_unit('btop'), 'flatpak\\firefox': flatpak_unit()}
    states = InstallState(fr).inspect(units)
    assert states['apt\\btop'].status == 'installed'
    assert states['flatpak\\firefox'].status == 'unsupported'
