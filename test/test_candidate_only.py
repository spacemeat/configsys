'''Driver-level `candidate-only` (drivers: { snap: { standing: never-auto } }): a via that is a
...
def test_snap_is_listed_but_not_the_ubuntu_default(tmp_path):
    r = _resolver(tmp_path, 'snap: { standing: never-auto }', BROWSER)
...
def test_binding_pin_still_forces_snap(tmp_path):
    r = _resolver(tmp_path, 'snap: { standing: never-auto }', BROWSER, pins={'browser': 'snap'})
    assert set(r.resolve_names(['browser'])) == {'snap\\browser'}


def test_candidate_only_wins_when_it_is_the_only_method(tmp_path):
    r = _resolver(tmp_path, 'snap: { standing: never-auto }', SNAPONLY)
