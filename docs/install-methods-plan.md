...
> **Status: HISTORICAL — SHIPPED. The per-binding `prefer:` it describes became `standing:` (docs/routing-model.md §8a).**
...
invariant, `driver-preference` + per-binding `prefer:` — since renamed `standing:`, the same-driver-only ambiguity gate),
